...
class Lexer:
    ...
    def SkipNullTokens(self, token, current) -> (bool,int):
        nullTolkens = [' ', '\n', ',']
    ...
    def SkipSingleComments(self, token, current, line) -> (bool, int):
        # Skip Hash Comments
        if token == "#":
            return True, len(line)
        return False, current

    def isBlockQuote(self, token, current, line) -> bool:
    ...
    '''
    def TokeniseCode(self,SourceCodeFilePath):
        operatorCount = 0;
        operandCount = 0;
            ...
            for sline in file:
                ...
                while current < len(line):
                    ...
                    if line[current] in blockQuoteChars or inQuoteBlock:
                        blockQuote = self.isBlockQuote(line[current], current, line)
                        if inQuoteBlock:
                            current = len(line)
                            if blockQuote:
                                inQuoteBlock = False
                    ...
                    # Skip null Tokens
                    valid, current = self.SkipNullTokens(line[current], current)
                    if valid:
                        continue;

                    # Skip Single Comments
                    valid, current = self.SkipSingleComments(line[current], current, line)
                    if valid:
                        continue;
                    ...
                    if line[current].isalpha() or line[current] == "_" or ((line[current] + line[current+1]) == "__"):
                        ...
                        while line[current].isalpha() or line[current] == "_" or line[current].isdigit():
                            word += line[current]
                            current += 1
                        if word in self.__OP_TABLE:
                            if word not in distinctOperators:
                                distinctOperators[word] = self.__OP_TABLE[word]
                            operatorCount += 1
                            current += 1
                        else:
                            operandCount += 1
                    ...
                    # Single Ops
                    if line[current] in self.__OP_TABLE:
                        if line[current] not in distinctOperators:
                            distinctOperators[line[current]] = self.__OP_TABLE[line[current]]
                        current += 1
                    else:
                        value = ""
                        while value not in self.__OP_TABLE:
                            value += line[current]
                            current += 1
