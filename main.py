...
if not testing:
    with open('results.csv', 'w', newline='') as file:
       ...
       writer.writerow(["Problem", "Temperature", "Distinct Operators", "Distinct Operands", "Total Operators", "Total Operands", "Vocabulary", "Length", "Estimated Program Length", "Volume", "Difficulty", "Effort", "Time", "Bugs Estimate"])
...
if testing:
    #response = Generation.GetResponce(problems['p2'], 0.6)
    Analyzer.PrintAllHalsteadMetrics("miscTestFile.py")

if not testing:
    for run in range(1, len(temperature)+1):
        # Generate Solutions
        for pNumber in enumerate(problems, start=1):
            for i in range(1, k+1):
                with open(f"Solutions/problem{pNumber}/p{pNumber}-{i}.py", "w") as file:
                    response = Generation.GetResponce(problems[pNumber], temperature[run-1])
                    print(f"temperature: {temperature[run-1]}")
                    file.write(response)
