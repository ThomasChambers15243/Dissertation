...
def ShowOperatorAndOperandStats(sourceCodeFilePath):
    ...
    Tokens = analysis.Analyser(sourceCodeFilePath)
    # Acquires the Halstead metrics as n1, n2, N1, N2
    distinctOperatorCount = len(Tokens.tokenCounts["distinctOperators"])
    distinctOperandCount = len(Tokens.tokenCounts["distinctOperands"])
    totalOperatorCount = Tokens.tokenCounts["operatorCount"]
    totalOperandCount = Tokens.tokenCounts["operandCount"]
    # Prints out above counts
    print(f"Number of Distinct Operators: {len(Tokens.tokenCounts['distinctOperators'])}\n")
    print(f"Number of Distinct Operands: {len(Tokens.tokenCounts['distinctOperands'])}\n")
    print(f"Total Number of Operators: {Tokens.tokenCounts['operatorCount']}\n")
    print(f"Total Number of Operands: {Tokens.tokenCounts['operandCount']}\n")
